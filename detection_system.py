...
from config import MODEL_PATH, CONFIDENCE_THRESHOLD, COUNT_LINE_Y, \
                   OVERLAY_X, OVERLAY_Y, OVERLAY_WIDTH, OVERLAY_HEIGHT, \
                   FONT_SCALE, FONT_THICKNESS, LINE_HEIGHT, CLASS_NAMES, OUTPUT_DIR
...
class VehicleDetectionSystem:
    ...
    def display_counts(self, frame, fps=0):
        ...
        for cid,cnt in self.entry_counts.items():
            cv2.putText(frame, f"{CLASS_NAMES[cid]}:{cnt}", (x+10, yy),
                        cv2.FONT_HERSHEY_SIMPLEX, FONT_SCALE*0.8, self.get_color(cid), 1)
            yy += LINE_HEIGHT-2
    ...
    def process_frame(self, frame):
        ...
        objs = self.tracker.update(rects, cids)
        ...
        for oid,(centroid,cid) in objs.items():
            ...
            if len(pts)>=2:
                ...
                if py<COUNT_LINE_Y<=cy and (oid,"in") not in self.counted_ids:
                    self.entry_counts[cid]+=1; self.counted_ids.add((oid,"in"))
                if py>COUNT_LINE_Y>=cy and (oid,"out") not in self.counted_ids:
                    self.exit_counts[cid]+=1; self.counted_ids.add((oid,"out"))
    ...
    def detect_video(self, path, window="Video"):
        ...
        while True:
            ...
            if key==ord("s"): self.save_session("video")
